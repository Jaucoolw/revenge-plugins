...
export default {
    onLoad: () => {
        ...
        const ChannelMessages =
            findByProps("ChannelMessages") ||
            findByName("ChannelMessages", false);
        ...
        // Make client-side VIEW_CHANNEL checks pass,
        // except while isHidden() is doing the real permission check.
        unpatches.push(
            after("can", Permissions, ([permID, channel], res) => {
                console.log(
                    "[HiddenChannels] Permissions.can called " +
                    (!channel?.realCheck &&
        ...
        if (Router?.transitionToGuild) {
            unpatches.push(
                instead("transitionToGuild", Router, (args, orig) => {
                    console.log(
                        "[HiddenChannels] Router.transitionToGuild called with args:",
                        args
                    );
        ...
        // UNCOMMENTED: prevent message fetching for hidden channels
        if (Fetcher?.fetchMessages) {
            unpatches.push(
                instead("fetchMessages", Fetcher, (args, orig) => {
                    console.log(
                        "[HiddenChannels] Fetcher.fetchMessages called with args:",
                        args
                    );
        ...
        // UNCOMMENTED: replace the message view for hidden channels
        if (ChannelMessages?.default) {
            unpatches.push(
                instead("default", ChannelMessages, (args, orig) => {
                    console.log(
                        "[HiddenChannels] ChannelMessages.default called with args:",
                        args
                    );
                    ...
                    console.log(
                        "[HiddenChannels] ChannelMessages.default called with:",
                        channel,
                        "isHidden:",
                        isHidden(channel)
                    );
            ...
            console.warn(
                "[HiddenChannels] ChannelMessages.default not found."
            );
        }
